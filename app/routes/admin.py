...
async def trigger_analysis(request: Request, body: AnalysisRequest | None = None):
    if not _check_admin(request):
...
async def analysis_status(request: Request, run_id: int):
    if not _check_admin(request):
        return JSONResponse({"detail": "Forbidden"}, status_code=403)
...
async def analysis_results(request: Request, run_id: int | None = None):
    if not _check_admin(request):
        return JSONResponse({"detail": "Forbidden"}, status_code=403)
