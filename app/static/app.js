...
function handleWSEvent(data) {
  ...
  if (data.type === "new_message") {
    ...
    loadConversations();
  } else if (data.type === "campaign_progress" || data.type === "campaign_status") {
    if (currentTab === "campaigns" && currentCampaignId === data.campaign_id) {
...
async function createCampaign() {
  ...
  const res = await fetch("/campaigns", { method: "POST", body: formData });
  if (!res.ok) {
    const err = await res.json();
...
async function loadCampaigns() {
  ...
  for (const c of campaigns) {
    const div = document.createElement("div");
    div.className = "conv-item" + (c.id === currentCampaignId ? " active" : "");
...
async function generateVariations(campaignId) {
  const btn = event.target;
  btn.disabled = true;
  btn.textContent = "Gerando...";
  try {
    const res = await fetch(`/campaigns/${campaignId}/generate-variations`, { method: "POST" });
    if (!res.ok) {
      const err = await res.json();
...
async function editVariation(campaignId, variationIdx, btnEl) {
  ...
  saveBtn.onclick = async () => {
    const res = await fetch(`/campaigns/${campaignId}/variations/${variationIdx}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ variation_text: textarea.value }),
    });
...
async function startCampaign(id) {
  ...
  await fetch(`/campaigns/${id}/start`, { method: "POST" });
...
async function retryCampaign(id) {
  ...
  await fetch(`/campaigns/${id}/retry`, { method: "POST" });
  await openCampaignDetail(id);
}
